...
public class GsiClearUp {
     ...
     */
    public GsiClearUp(List<String> lines) {
     ...
     */
    public List<String> process(boolean holdStations, boolean holdControlPoints) {
    ...
    private boolean checkIsControlPoint(String line) {
        final String controlPointIdentifier = Main.pref.getUserPreference(PreferenceKey.PARAM_CONTROL_POINT_STRING);
    ...
    private boolean checkIsStationLinePointBased(String line) {
        ...
        String freeStationIdentifier = "0000" + Main.pref.getUserPreference(PreferenceKey.PARAM_FREE_STATION_STRING);
        String stationIdentifier = "0000" + Main.pref.getUserPreference(PreferenceKey.PARAM_KNOWN_STATION_STRING);
     ...
     * @return cleared up measurement file
     */
    private List<String> processPointBasedClearUp(boolean holdStations, boolean holdControlPoints) {
        ...
        if (!holdStations) {
            ...
            lines = removeDuplicateStationLines(lines);
        }

        // Use a helper array to identify the different lines by 'type'.
        final lineType[] helperArray = new lineType[lines.size()];
     ...
     * @return cleared up measurement file
     */
    private List<String> processStationBasedClearUp(boolean holdStations, boolean holdControlPoints) {
        List<String> result = new ArrayList<>();

        // Remove directly sequent station lines
        List<String> reduced;
        if (!holdStations) {
            reduced = removeDuplicateStationLines(lines);
        } else {
            reduced = new ArrayList<>(lines);
        }
    ...
    private List<String> removeDuplicateStationLines(List<String> lines) {
